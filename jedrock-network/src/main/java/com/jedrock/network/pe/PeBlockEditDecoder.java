...
import static com.jedrock.network.pe.McpeProtocol.ACTION_CONTINUE_BREAK;
import static com.jedrock.network.pe.McpeProtocol.ACTION_START_BREAK;
...
final class PeBlockEditDecoder {
    ...
    /** A decoded world edit: the target cell and the new canonical block id (AIR = break). */
    record BlockEdit(int x, int y, int z, int blockId) {}
    ...
    static BlockEdit decodeUseItem(ByteBuf pk) {
        try {
            ...
            int itemId = McpeCodec.readItemId(pk);  // block in hand

            if (bx == 0 && by == 0 && bz == 0) {
                return null;                        // sync echo
            }
            return place(bx, by, bz, face, itemId);
     ...
     * face. In creative, breaking a block is instant, so a start/continue-break action on a block
     * means "remove it". This is how the 1.1.5 Win10 client reports breaks (not via transaction).
     */
    static BlockEdit decodePlayerAction(ByteBuf pk) {
        try {
            ...
            return isBreak ? new BlockEdit(bx, by, bz, Blocks.AIR) : null;
        } catch (RuntimeException e) {
    ...
    static BlockEdit decodeInventoryTransaction(ByteBuf pk) {
        try {
            ...
            int face = ByteBufUtils.readSignedVarInt(pk);
            ByteBufUtils.readSignedVarInt(pk);              // hotbar slot — unused
            int itemId = McpeCodec.readItemId(pk);          // block in hand (for placement)
     ...
     */
    private static BlockEdit place(int bx, int by, int bz, int face, int itemId) {
        if (!Blocks.isKnown(itemId) || itemId == Blocks.AIR) {
            return null;
        }
        int f = (face >= 0 && face < 6) ? face : 1;
        return new BlockEdit(bx + FACE_DX[f], by + FACE_DY[f], bz + FACE_DZ[f], itemId);
    }
