...
- ✅ **Sidebar scoreboard and boss bar (Java), plus virtual chests for scripts.** Three illusion-toolkit
  ...
  bar across the top (1.12.2's dedicated packet). Both are Java-only for now — 1.8 has the scoreboard but no
  boss-bar packet, and Bedrock's legacy clients aren't wired here (0.14 predates both; 1.1.5 is a follow-up
  against a real client). And **`menus`** gives scripts a **virtual chest**: `menus.create(title, rows)`,
  laid out with `setItem`, opened with `open(player)` — with an `onClick` it's a read-only **button menu**
  (a class picker, a shop), without one a transient **storage chest**. Java only, since the 1.1.5 client
  crashes on a chest window. Packet ids are from minecraft-data; on-client behaviour isn't verified here.
  Try `/sb on`, `/boss 50 red` and `/menu` in `plugins/example.js`.
...
- **The platform API — the centrepiece (in progress).** Turn `api` from a thin contract into a real
  ...
  **sidebar scoreboard** and a **boss bar** (Java), plus **virtual chests** for scripts (the `menus`
  global). What it still wants: those three on **Bedrock 1.1.5** (against a real client) — **forms** stay
